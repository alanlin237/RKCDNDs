...
json_file1 = open('13194148204544model89.json','r');
json_model1 = json_file1.read();
net43 = model_from_json(json_model1);
net43.load_weights('13194148204544best89depth.h5');


m50 = np.load('data_wrong_key_mean_106r.npy');
s50 = np.load('data_wrong_key_std_106r.npy'); s50 = 1.0/s50;
m43 = np.load('data_wrong_key_mean_99r.npy');
s43 = np.load('data_wrong_key_std_99r.npy'); s43 = 1.0/s43;
...
def gen_key(nr):
  ...
  ks1 = 0x0^(all_key[194]<<15)^(all_key[196]<<14)^(all_key[198]<<13)^(all_key[199]<<12)^(all_key[200]<<11)^(all_key[201]<<10)^(all_key[202]<<9)^(all_key[203]<<8)^(all_key[204]<<7)^(all_key[205]<<6)^(all_key[206]<<5)^(all_key[207]<<4)^(all_key[208]<<3)^(all_key[209]<<2)^(all_key[210]<<1)^(all_key[211]);  
  ks2 = 0x0^(all_key[180]<<15)^(all_key[182]<<14)^(all_key[184]<<13)^(all_key[185]<<12)^(all_key[186]<<11)^(all_key[187]<<10)^(all_key[188]<<9)^(all_key[189]<<8)^(all_key[190]<<7)^(all_key[191]<<6)^(all_key[192]<<5)^(all_key[193]<<4)^(all_key[194]<<3)^(all_key[195]<<2)^(all_key[196]<<1)^(all_key[197]);
  return(keys,ks1,ks2);

def gen_challenge(n, nr, diff=(0xC0000845000), keyschedule='real'):
...
def verifier_search(cts, best_guess, use_n = 48, net = net43):
  #print(best_guess);
  #print(cts)
...
def test_bayes(cts,it=1, cutoff1=10, cutoff2=10, net=net50, net_help=net43, m_main=m50, m_help=m43, s_main=s50, s_help=s43, verify_breadth=None):
  n = 32;
  if (verify_breadth is None): verify_breadth=48;
  alpha = sqrt(n);
...
def test(n, nr=106, num_structures=1000, it=500, cutoff1=5.0, cutoff2=50.0, keyschedule='real',net=net50, net_help=net43, m_main=m50, s_main=s50,  m_help=m43, s_help=s43, verify_breadth=None):
