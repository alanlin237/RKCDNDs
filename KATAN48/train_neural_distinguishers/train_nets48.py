...
def make_resnet(k, num_blocks=2, num_filters=48, num_outputs=1, d1=64, d2=64, word_size=32, ks=3, reg_param=0.0001, final_activation='sigmoid'):
  #Input and preprocessing layers
  inp = Input(shape=(48*int(k),))
  rs = Reshape((48, int(k)))(inp)
...
def train_katan_distinguisher(num_epochs, Round, diff):
    ...
    #X, Y = ka.make_train_data1(4*10**7,Round,diff)
    #X_eval, Y_eval = ka.make_train_data1(4*10**6, Round,diff)
...
D = 0
for i in range(3):
    val_acc=train_katan_distinguisher(20, a[i], 0xC0000845000,48)
    if(val_acc>M):
        M=val_acc
        D=a[i]
    print(M,D,sep='  ')
